<template>
  <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/30 p-2 sm:p-4">
    <div
      class="bg-white rounded-3xl shadow-2xl w-full max-w-[90vw] lg:max-w-[1000px] p-4 sm:p-6 md:p-8 relative overflow-y-auto max-h-[95vh] modal-root-container"
    >
      <button
        ...
        <div class="flex flex-col gap-3 sm:gap-4">
          ...
          <div class="grid grid-cols-7 gap-0.5 sm:gap-1 text-center">
            <div
              v-for="d in days"
              :key="d"
              class="font-semibold text-gray-500 mb-1 text-xs sm:text-sm flex items-center justify-center h-7 sm:h-8"
            >
              {{ d }}
            </div>
            <div
              ...
              :key="idx"
              class="day-cell-wrapper w-full aspect-square flex items-center justify-center cursor-pointer transition-all duration-150 text-xs sm:text-sm"
              :class="[
...
const props = defineProps({
  visible: Boolean,
  selectedDate: String // รับค่า selectedDate จากคอมโพเนนต์แม่
})
